...
mod_ab0 = read_xlsx('input/models/etab_modelsDE.xlsx') |> data.table()
...
# make grps
mod_ab0[, grp_fe := gsub('.*(arbnr.*$)', '\\1', grp) |> factor() ]
mod_ab0[, grp_mod := gsub('(.*)_arbnr.*$', '\\1', grp) |> factor() ]
...
# make a regex that grabs  everything after the last underscore _ in the following:
# "gendered_man_dummy_arbnr"            "gendered_man_dummy_arbnr_disco6"
...
mod_ab1[term0 %ilike% 'female|bin_', .N, term0]
mod_ab1[term0 %ilike% 'female|bin_', term := term0 ]
...
plot_ab2a = plot_ab1[ grp_mod %ilike% 'nogendered' & !is.na(coef1) & grp_fe %ilike% 'disco']
...
plot_ab2b = plot_ab1[ grp_mod %ilike% '^gendered' & !is.na(coef1) ]
# make a coef plot with conf intervals in ggplot and x axis should have 0.025 as tick marks
...
# having established how close these models are to each other, we pick the disco 3 and the binary degree and the arbhr pure

plot_ab2c = plot_ab1[( term %ilike% '^female$' | term %ilike% 'non-mgnr.*|mgnr.*tie' )   & grp_fe %ilike% '^arbnr$|disco[6]' &
grp_mod %in% qc(all)]
plot_ab2c[, .N, grp_fe]


# make a coef plot with conf intervals in ggplot
ggplot(plot_ab2c, aes(coef1, term, group=grp_fe, fill=grp_fe, color=grp_fe)) + 
geom_point(size=4, position= position_dodge(width=0.5)) + 
# geom_linerange(aes(xmin=coef1-sd1, xmax=coef1+sd1), linewidth=2) + 
geom_errorbar(aes(xmin=coef1-sd1, xmax=coef1+sd1), width=0.1, position=position_dodge(width=0.5)) + 
# facet_wrap(~grp_fe, scales='free_y') +
# theme(legend.position='none') + 
labs(x='coef', y='term') +
# a vertical line at zero 
geom_vline(xintercept=0, color='black') +
geom_vline(xintercept=0, color='black') +
theme_bb() + scale_x_continuous(breaks= seq(-0.1, 0.1, 0.025)) +
coord_cartesian(xlim=c(-0.075, 0.05)) 
# plotly::ggplotly()



plot_ab2d = plot_ab1[( term %ilike% '^female$' | term %ilike% 'non-mgnr.*|mgnr.*tie' )   & grp_fe %ilike% 'disco[6]' &
grp_mod %in% qc(all) & term %!in% 'female']
# lav en ny gruppe med de to mgnr og de to non-mgnr
plot_ab2d[, grp_tietype := gsub('.*mgnr (f*e*male tie.*)', '\\1', term)][, .(term, grp_tietype)]
plot_ab2d[, .N, grp_tietype]
plot_ab2d[, grp_mgrtype := gsub('(non-mgnr|mgnr).*', '\\1', term)][, .(term, grp_mgrtype, grp_tietype)]
plot_ab2d[, .N, grp_mgrtype]


# make a coef plot with conf intervals in ggplot
ggplot(plot_ab2d, aes(coef1, grp_tietype, group=grp_mgrtype, fill=grp_mgrtype, color=grp_mgrtype)) + 
geom_point(size=4, position= position_dodge(width=0.5)) + 
# geom_linerange(aes(xmin=coef1-sd1, xmax=coef1+sd1), linewidth=2) + 
geom_errorbar(aes(xmin=coef1-sd1, xmax=coef1+sd1), width=0.1, position=position_dodge(width=0.5)) + 
# facet_wrap(~grp_mgrtype, scales='free_y') +
# theme(legend.position='none') + 
labs(x='coef', y='term') +
# a vertical line at zero 
geom_vline(xintercept=0, color='black') +
